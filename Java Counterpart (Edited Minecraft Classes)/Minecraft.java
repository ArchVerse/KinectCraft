...
public abstract class Minecraft implements Runnable, IPlayerUsage
{
    ...
    public WoollySocks socks;
    public boolean kinectActive = false;
        ...
        try
        {
            while (this.running)
            {
            	if(kinectActive)
            	{
            		...
            		String fsStr = (this.fullscreen) ? "true" : "false";
            		//this.socks.sendMessage("fullscreen[" + fsStr + "]");
            	}
    ...
    public void loadWorld(WorldClient par1WorldClient, String par2Str)
    {
        ...
        if (par1WorldClient != null)
        {
            ...
            this.thePlayer.movementInput.socks.messageCallbacks.add(new NewMessage() {
            	...
            	public void sendMessage(String message)
            	{
        			...
        			{
        				if(instructions[i].compareTo("attack") == 0)
        				{
        					clickMouse(0);
        					gameSettings.keyBindAttack.pressed = true;
        				}
        				if(instructions[i].compareTo("!attack") == 0)
        				{
        					gameSettings.keyBindAttack.pressed = false;
        				}
        				if(instructions[i].compareTo("use") == 0)
        				{
        				...
        				if(instructions[i].compareTo("spinMode[spinOnly]") == 0)
        				{
        					ingameGUI.getChatGUI().printChatMessage("Turn Mode: [Spin]");
        				}
        				if(instructions[i].compareTo("spinMode[look]") == 0)
        				{
        					ingameGUI.getChatGUI().printChatMessage("Turn Mode: [Look]");
        				}
        				if(instructions[i].compareTo("skelLeft") == 0)
        				{
        					ingameGUI.getChatGUI().printChatMessage("Player Left Kinect Vision Range: Switching Back to Mouse and Keyboard");
        					kinectActive = false;
        				}
        				if(instructions[i].compareTo("ready1") == 0)
        				{
        					ingameGUI.getChatGUI().printChatMessage("Ready [1] : Kinect Paused");
        					kinectActive = false;
        				}
        				if(instructions[i].compareTo("ready2") == 0)
        				{
        					ingameGUI.getChatGUI().printChatMessage("Ready [2] : Position Reset, Kinect Activated");
        					kinectActive = true;
        					thePlayer.rotationPitch = 0.0F;
        				}
        				if(instructions[i].compareTo("ready3") == 0)
        				{
        					thePlayer.rotationPitch = 1.0F;
        				}
        			}
            	}
            ...
            this.socks = this.thePlayer.movementInput.socks;
